...
class Supervisor_model extends CI_Model{
	...
	public function agendadoSemanaPendente(){
		$this->db->trans_start();
		$this->db->query("SET lc_time_names = 'pt_BR'");
		$result = $this->db->query("SELECT SUBSTRING(DAYNAME(STR_TO_DATE(dt_processo, '%d/%m/%Y')),1, 3) AS 'dia', COUNT(dt_processo) AS 'contagem' FROM `tb_candidato` WHERE WEEKOFYEAR(STR_TO_DATE(dt_processo, '%d/%m/%Y')) = WEEKOFYEAR(now())  AND id_status = 4 GROUP BY dt_processo");
		$this->db->trans_complete();
		return $result;
	}
	
	public function realizadoSemana(){
		$this->db->trans_start();
		$this->db->query("SET lc_time_names = 'pt_BR'");
		$result = $this->db->query("SELECT SUBSTRING(DAYNAME(STR_TO_DATE(dt_processo, '%d/%m/%Y')),1, 3) AS 'dia', COUNT(dt_processo) AS 'contagem' FROM `tb_candidato` WHERE WEEKOFYEAR(STR_TO_DATE(dt_processo, '%d/%m/%Y')) = WEEKOFYEAR(now()) AND id_status != 4 GROUP BY dt_processo");
		$this->db->trans_complete();
